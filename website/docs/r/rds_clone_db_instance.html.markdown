...
For information about RDS Clone DB Instance and how to use it, see [What is Clone DB Instance](https://help.aliyun.com/).
...
resource "alicloud_rds_clone_db_instance" "example" {	
  source_db_instance_id       = "your source db_instance_id"
...
* `instance_charge_type` - (Required) The billing method of the new instance. Valid values:
  * **Postpaid**: pay-as-you-go
  * **Prepaid**: subscription.
* `instance_type` - (Optional) The instance type of the new instance. For information, see [Primary ApsaraDB RDS instance types](https://www.alibabacloud.com/doc-detail/26312.htm).
...
* `instance_storage` - (Optional) The storage capacity of the new instance. Unit: GB. The storage capacity increases in increments of 5 GB. For more information, see [Primary ApsaraDB RDS instance types](https://www.alibabacloud.com/doc-detail/26312.htm).
...
* `private_ip_address` - (Optional) The intranet IP address of the new instance must be within the specified vSwitch IP address range. By default, the system automatically allocates by using **VPCId** and **VSwitchId**.
...
* `acl` - (Optional) This parameter is only supported by the RDS PostgreSQL cloud disk version. This parameter indicates the authentication method. It is allowed only when the public key of the client certificate authority is enabled. Valid values: `cert` and `perfer` and `verify-ca` and `verify-full (supported by RDS PostgreSQL above 12)`.
* `auto_upgrade_minor_version` - (Optional) How to upgrade the minor version of the instance. Valid values:
...
* `ca_type` - (Optional) This parameter is only supported by the RDS PostgreSQL cloud disk version. It indicates the certificate type. When the value of sslenable is 1, the default value of this parameter is aliyun. Value range:
  * **aliyun**: using cloud certificates
  * **custom**: use a custom certificate. Valid values: `aliyun`, `custom`.
* `category` - (Optional, ForceNew) Instance series. Valid values:
  * **Basic**: Basic Edition
  * **HighAvailability**: High availability
  * **AlwaysOn**: Cluster Edition
  * **Finance**: Three-node Enterprise Edition.
* `certificate` - (Optional) The file that contains the certificate used for TDE.
* `client_ca_cert` - (Optional) This parameter is only supported by the RDS PostgreSQL cloud disk version. It indicates the public key of the client certification authority. If the value of clientcaebable is 1, this parameter must be configured.
* `client_ca_enabled` - (Optional) The client ca enabled.
* `client_cert_revocation_list` - (Optional) This parameter is only supported by the RDS PostgreSQL cloud disk version, which indicates that the client revokes the certificate file. If the value of clientcrlenable is 1, this parameter must be configured.
* `client_crl_enabled` - (Optional) The client crl enabled.
* `connection_string_prefix` - (Optional) The connection string prefix.
* `instance_name` - (Optional) The db instance description.
...
* `encryption_key` - (Optional) The key ID of the disk encryption in the same region. You can view the key ID or create a new key in the key management service console. For more information, see [Management Key](~~ 108805 ~~).
> This parameter indicates that cloud disk encryption is enabled (cannot be turned off after it is enabled), and **RoleARN** is required * *.
* `engine_version` - (Optional) Database version. Value:
  * MySQL:**5.5/5.6/5.7/8.0**
  * SQL Server:**2008r2/08r2_ent_ha/2012/2012_ent_ha/2012_std_ha/2012_web/2014_std_ha/2016_ent_ha/2016_std_ha/2016_web/2017_std_ha/2017_ent/2019_std_ha/2019_ent**
  * PostgreSQL:**9.4/10.0/11.0/12.0/13.0**
  * PPAS:**9.3/10.0**
  * MariaDB:**10.3**.
* `general_group_name` - (Optional) The general group name.
* `instance_network_type` - (Optional, ForceNew) The network type of the instance. Valid values:
  * **Classic**: Classic Network
  * **VPC**: VPC.
* `maintain_time` - (Optional) The maintainable time period of the instance. Format: <I> HH:mm</I> Z-<I> HH:mm</I> Z(UTC time).
* `pass_word` - (Optional) The password of the certificate. 
...
* `replication_acl` - (Optional) This parameter is only supported by the RDS PostgreSQL cloud disk version, indicating the authentication method of the replication permission. It is only allowed when the public key of the client certificate authority is enabled. Valid values: `cert` and `perfer` and `verify-ca` and `verify-full (supported by RDS PostgreSQL above 12)`.
...
* `security_ips` - (Optional) The IP address whitelist of the instance. Separate multiple IP addresses with commas (,) and cannot be repeated. The following two formats are supported:
...
> each instance can add up to 1000 IP addresses or IP segments, that is, the total number of IP addresses or IP segments in all IP whitelist groups cannot exceed 1000. When there are more IP addresses, it is recommended to merge them into IP segments, for example, 10.23.12.0/24.
* `server_cert` - (Optional) This parameter is only supported by the RDS PostgreSQL cloud disk version. It indicates the content of the server certificate. If the CAType value is custom, this parameter must be configured.
* `server_key` - (Optional) This parameter is only supported by the RDS PostgreSQL cloud disk version. It indicates the private key of the server certificate. If the value of CAType is custom, this parameter must be configured.
* `source_biz` - (Optional) The source biz.
* `ssl_action` - (Optional) Enable or disable SSL. Valid values: `Open` and `Close`
...
* `ha_mode` - (Optional) The high availability mode. Valid values:
...
* `sync_mode` - (Optional) [The data replication mode](https://www.alibabacloud.com/help/doc-detail/96055.htm). Valid values:
  * **Sync**: strong synchronization
  * **Semi-sync**: Semi-synchronous
  * **Async**: asynchronous

> SQL Server 2017 cluster version is currently not supported. 
...
* `whitelist_network_type` - (Optional) The whitelist network type.
* `zone_id` - (Optional) The ID of the zone to which the new instance belongs. You can call the [DescribeRegions](https://www.alibabacloud.com/doc-detail/26243.htm) operation to query the most recent region list.
...
> The parameter **DBInstanceNetType** determines whether the address is internal or public.
...
* `create` - (Defaults to 2 mins) Used when create the Clone DB Instance.
