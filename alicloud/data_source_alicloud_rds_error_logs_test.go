...
func TestAccAlicloudRdsErrorLogsDataSource(t *testing.T) {
	rand := acctest.RandInt()

	testConf := dataSourceTestAccConfig{
		existConfig: testAccCheckAlicloudRdsErrorLogsDataSourceName(rand, map[string]string{
			"db_instance_id": `"${alicloud_db_instance.default.id}"`,
			"start_time":     `"${var.startTime}"`,
			"end_time":       `"${var.endTime}"`,
		}),
		fakeConfig: testAccCheckAlicloudRdsErrorLogsDataSourceName(rand, map[string]string{
			"db_instance_id": `"${alicloud_db_instance.default.id}"`,
			"start_time":     `"${var.startTime}"`,
	...
	var alicloudRdsLogsCheckInfo = dataSourceAttr{
		resourceId:   "data.alicloud_rds_error_logs.default",
...
func testAccCheckAlicloudRdsErrorLogsDataSourceName(rand int, attrMap map[string]string) string {
	var pairs []string
...
data "alicloud_rds_error_logs" "default" {
  %s
